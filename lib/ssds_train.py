...
class Solver(object):
    ...
    def train_model(self):
        ...
        for epoch in iter(range(start_epoch + 1, self.max_epochs + 1)):
            ...
            if 'train' in cfg.PHASE:
                print("train epoch")
                if 'mixup' in cfg.TRAIN_MODE:
                    print("#####  Using Mixup  ######")
                    self.mixup_train_epoch(self.model, self.train_loader, self.optimizer, self.criterion, self.writer,
                                           epoch, self.use_gpu)
                else:
            ...
            if 'eval' in cfg.PHASE:
                ...
                self.eval_epoch(self.model, self.eval_loader, self.detector, self.criterion, self.writer, epoch,
                                self.use_gpu)
            if 'test' in cfg.PHASE and epoch % cfg.TEST_STEP == 0:
                print("test epoch")
                self.test_epoch(self.model, self.test_loader, self.detector, self.output_dir, self.writer, epoch,
                                self.use_gpu)
    ...
    def train_epoch(self, model, data_loader, optimizer, criterion, writer, epoch, use_gpu):
        ...
        for iteration in iter(range((epoch_size))):
            ...
            # print("load image...")
            # assert images is not None, "Image None Error......"
            if use_gpu:
            ...
            if loss_l.data[0] >= 9999999.0:
                # if loss_l.data[0] == np.inf or loss_l.data[0] == float("Inf"):
                print("Skip?")
                continue
    ...
    ###           Mixup                             ##
    ##################################################
    def mixup_train_epoch(self, model, data_loader, optimizer, criterion, writer, epoch, use_gpu, alpha=1.0):
        model.train()
        epoch_size = len(data_loader)
        batch_iterator = iter(data_loader)
        print("start load data")
        ...
        for iteration in iter(range((epoch_size))):
            ...
            if use_gpu:
                # index = torch.randperm(cfg.DATASET.TRAIN_BATCH_SIZE).cuda()
                images = Variable(images.cuda())
                targets = np.array([Variable(anno.cuda(), volatile=True) for anno in targets])
            else:
                images = Variable(images)
                targets = np.array([Variable(anno, volatile=True) for anno in targets])
        ...
        if cfg.DATASET.DATASET == 'voc':
            ...
            for cls_ind, cls in enumerate(VOC_CLASSES):
                if cls == '__background__':
                    continue
                txt = "Test/" + cls
                writer.add_scalar(txt, aps[cls_ind - 1], epoch)
