...
class Pelee(nn.Module):
    """Pelee Net
    See: https://arxiv.org/pdf/1804.06882.pdf
    """

    def __init__(self, base, extras, head, feature_layer, num_classes):
        super(Pelee, self).__init__()
    ...
    def _dense_block(self, x, in_channel, out_channel, ):
        x = nn.Conv2d()
...
def build_ssd(base, feature_layer, mbox, num_classes):
    ...
    """
    assert base is None, "Now Pelee implementation only support Basic Net"
    base_, extras_, head_ = add_extras(None, feature_layer, mbox, num_classes, version='ssd')
    return Pelee(base_, extras_, head_, feature_layer, num_classes)
