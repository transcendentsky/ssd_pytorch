...
class _conv_bn(nn.Module):
    def __init__(self, inp, oup, stride, activation='lrelu'):
...
class _conv_block(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio=0.5, activation='lrelu'):
...
def darknet(conv_defs, depth_multiplier=1.0, min_depth=8, bn=True):
...
def darknet_swish(conv_defs, depth_multiplier=1.0, min_depth=8, activation='lrelu'):
    depth = lambda d: max(int(d * depth_multiplier), min_depth)
    layers = []
    in_channels = 3
    for conv_def in conv_defs:
        if isinstance(conv_def, Conv):
            layers += [_conv_bn(in_channels, depth(conv_def.depth), conv_def.stride, activation=activation)]
            in_channels = depth(conv_def.depth)
        elif isinstance(conv_def, ConvBlock):
            for n in range(conv_def.num):
                stride = conv_def.stride if n == 0 else 1
                layers += [_conv_block(in_channels, depth(conv_def.depth), stride, conv_def.t, activation=activation)]
                in_channels = depth(conv_def.depth)
        elif isinstance(conv_def, ResidualBlock):
            for n in range(conv_def.num):
                stride = conv_def.stride if n == 0 else 1
                layers += [
                    _residual_block(in_channels, depth(conv_def.depth), stride, conv_def.t, activation=activation)]
                in_channels = depth(conv_def.depth)
        elif conv_def == 'M':
            layers += [nn.MaxPool2d(kernel_size=2, stride=2)]
    return layers
...
darknet_19_nobn = wrapped_partial(darknet, conv_defs=CONV_DEFS_19, depth_multiplier=1.0, bn=False)

darknet_19_swish = wrapped_partial(darknet_swish, conv_defs=CONV_DEFS_19, depth_multiplier=1.0, activation='swish')